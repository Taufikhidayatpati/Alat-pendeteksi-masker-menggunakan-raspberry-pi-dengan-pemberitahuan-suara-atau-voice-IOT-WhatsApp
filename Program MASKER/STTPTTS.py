...
while True:
    ...
    for (box, pred) in zip(locs, preds):
        ...
        if label == "Masker":
            red.off()
            green.on()
            a = "Terima kasih telah memakai masker. Jangan lupa menjaga jarak"
            
            # speak aloud
            c = 'espeak -vid+f2 -k19 -s150 --punct="<characters>" "%s" 2>>/dev/null --stdout|aplay' % a 
            execute_unix(c) 
            red.off()
            green.off()
        else:
            ...
            # speak aloud
            c = 'espeak -vid+m3 -k19 -s150 --punct="<characters>" "%s" 2>>/dev/null --stdout|aplay' % a 
            execute_unix(c)
            green.off()
            red.off()
